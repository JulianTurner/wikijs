...
### Beobachtungen und stochastische Modelle
...
- charakterisiert Population
- lernen mit Zufallsstichprobe
- Schätzung repräsentiert Wissen über Population
...
## Wie kann man die Wahrscheinlichkeit eines Ereignisses finden?
...
> Spielt die Reihenfolge / Züge eine Rolle?

Reihenfolge von mehreren Ereignissen:  
...
Kleine Population + ziehen ohne zurücklegen => reduziert Unsicherheit
Große Population + ziehen ohne zurücklegen => ähnlich wie ziehen mit zurücklegen

## Diskrete Warscheinlichkeitsverteilungen

### Diskrete Zufallsvariablen und diskrete Verteilung
...
Der Erwartungswert ist eine Maßzahl zur Charakterisierung einer Wahrscheinlichkeitsverteilung.
...
Wenn ein binomialverteiltes Experiment aus n Versuchen besteht, wobei jeder Versuch eine Wahrscheinlichkeit von p hat, dann ist die Wahrscheinlichkeit für k Erfolge:

$P(X=k) = \binom{n}{k}*p^k* (1-p)^{n-k}$  

Der Binominialkoeffizient $\binom{n}{k}$ berechnet für uns die Anzahl der Möglichkeiten, wie k Objekte in einer Gruppe aus n ohne Wiederholung angeordnet werden können.

Beispiel Befragung:
$n$ = Alle Befragten  
$p$ = Wahrscheinlichkeit (zwischen 0 und 1; 1 = 100%)  
$k$ = Anzahl der Erfolge

> $X \sim B(n, p)$
> Die Zufallsvariable $X$ folgt der Binomialverteilung mit den Parametern $n$ und $p$
...
![Normalverteilung](https://viles.uni-oldenburg.de/navtest/viles2/kapitel02_Theoretische~~lVerteilungen/modul02_Normalverteilung/ebene01_Konzepte~~lund~~lDefinitionen/STNV.png)
&#8203;

#### Standardisierung

> Nur für die Standardnormalverteilung gibt es eine Tabelle (Formelsammlung s. 13)  
Wenn $X \sim N(\mu, \sigma^2) \neq N(0,1)$, dann können wir $X$ standardisierten  
...
Eine normalverteilte Zufallsvariable beobachtet an $n$ verschiedenen unabhängigen Objekten.
...
> => arithmetisches Mittel ist ebenfalls normalverteilt $\bar{X}=\frac{1}{n}\displaystyle\sum_{i=1}^{n}X_i \sim N(\mu,\frac{\sigma^2}{n})$  
&#8203;
...
- Verteilung nennt man **stetige Wahrscheinlichkeitsverteilung**

Beispiele:

- Tagesrendite beim DAX
- Reinheitsgrad einer produzierten Chemie

### Berechnung von Wahrscheinlichkeiten

Wahrscheinlichkeitsfunktion für stetige Zufallsvariablen ist **unbrauchbar**.

> Man braucht die Dichte

### Dichte

Wahrscheinlichkeiten sind Flächen unter der Dichte  
Formel für die stetige Wahrscheinlichkeitsverteilung:  
...
iid => "independently identically distributed" (unabhängig & gleichmäßig verteilt)
...
### Binomialverteilung $B(n,p)$ bei großen $n$&#8203;
...
> Die Approximation ist gut falls für die Varianz $np(1-p)$ gilt:  
> $np(1-p) > 9$  
